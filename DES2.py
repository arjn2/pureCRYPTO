...
def enc(txt,key):
    if len(key)<=len(txt):
        ...
        while (len(txtbin)%64)!=0:
            txtbin='0'+txtbin
        while len(keybin)<len(txtbin):
            keybin='10'+keybin
        
        keybin=keybin.zfill(len(txtbin))
        txtbinA=textwrap.wrap(txtbin,64)
        keybinA=textwrap.wrap(keybin,64)
    
        print('----------------------------')
    
        for i,j in zip(txtbinA,keybinA):
            ct+=bintotxt(encrypt(i,j))
        return ct

    else:
        print('Key size limited to plaintext-size')
...
def dec(txt,key):
    if len(key)<=len(txt):
        ...
        for i,j in zip(txtbinA,keybinA):
            pt+=bintotxt(decrypt(i,j))
        return pt
    else:
        print('Key size limited to plaintext-size')

    
ch=str(input("Enter a choice 1-encryption & decryption ::: 2-decryption only:(1 or 2):"))
...
if(ch=='1'):
    ...
    txt=str(input("Enter cipher-text"))
    key=str(input("Enter key"))
...
# print("\ndecipher:",decrypt(ct,'10'*32))
    ...
    # print("CIPHERBIN",txtbinA)
    # print("KEYBIN",keybinA)

# ct=enc('pkypakalkuri123/!#','good@!23')
# print("\nciphertext:",ct,len(ct))
# pt=dec(ct,'good@!23')
# print("\ndeciphertext:",pt,len(pt))
